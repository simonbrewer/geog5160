...
## -----------------------------------------------------------------------------------
current.env = stack(subset(current.env, seq(1,11)) / 10, 
                    subset(current.env, seq(12,19)))
# summary(current.env)
...
x = circles(pe_pres, d=100000, lonlat=T)

# draw random points that must fall within the circles in object x
pe_abs = spsample(x@polygons, nrow(pe_pres), type='random', iter=1000)

plot(pe_pres, pch=21, bg="darkorange")
...
rr = resample(task = task_pe, learner = at_ct, resampling = resampling_outer)
rr$score(measure)
rr$aggregate(measure)

at_ct$train(task_pe)

pred_test = at_ct$predict(task_pe, row_ids = test_set)
pred_test$score(measure)
autoplot(pred_test, type = 'roc')
...
rr = resample(task = task_pe, learner = at_rf, resampling = resampling_outer)
rr$score(measure)
rr$aggregate(measure)

at_rf$train(task_pe)

pred_test = at_rf$predict(task_pe, row_ids = test_set)
pred_test$score(measure)
autoplot(pred_test, type = 'roc')
...
rr = resample(task = task_pe, learner = at_brt, resampling = resampling_outer)
rr$score(measure)
rr$aggregate(measure)

at_brt$train(task_pe)

pred_test = at_brt$predict(task_pe, row_ids = test_set)
pred_test$score(measure)
autoplot(pred_test, type = 'roc')
...
pe.curr.pred = at_brt$predict_newdata(current.env.df)
...
# pe.rcp85.pred = at_brt$predict_newdata(rcp85.env.df) ## Doesn't work
names(rcp85.env.df) <- names(current.env.df)
pe.rcp85.pred = at_brt$predict_newdata(rcp85.env.df)
