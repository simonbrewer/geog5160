...
### Using an different movement routine

An alternative method for moving agents is as a random walk, rather than movement across a gradient. We'll add a second movement routine to illustrate how to simulate this type of movement. Random walks are decomposed into two parts: *turn* and *move*. Here we allow the agents to turn randomly to the left or right by an amount between +/- 45 degrees (`lt`), and move forward (`fd`) by a random amount between 0 and 1. Note that we still use `q` to control the overall movement. If the first random number is less than `q`, the turtle turns toward the patch with the highest elevation. If not, then it follows the random walk described above. The overall effect is to move the turtles toward a target, but with a degree of stochasticity in their movement

Add the follow code as a new procedure `move-rnd`:
...
To use this, all we need to do is replace this line in the `go` procedure
...
----
Simon Brewer and Kenneth Dudley 02/09/2014; Updated 01/19/18 S.Brewer; Moved to html and added random walk 01/07/20
