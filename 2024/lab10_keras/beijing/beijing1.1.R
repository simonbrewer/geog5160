...
model_recipe_steps <- model_recipe %>% 
  ...
  step_lag(all_numeric(), lag = c(3, 2, 1))
...
for (i in 1:n_obs) {
  if (i %% 100 == 0) {print(i)}
  xx <- abind(xx, matrix(scaled[i, 9:ncol(scaled)], nrow = 3, byrow = TRUE), along = 3)
}
X_train <- aperm(xx, c(3,1,2))
...
xx <- NULL
for (i in n_obs:nrow(scaled)) {
  if (i %% 100 == 0) {print(i)}
  xx <- abind(xx, matrix(scaled[i, 9:ncol(scaled)], nrow = 3, byrow = TRUE), along = 3)
}
X_test <- aperm(xx, c(3,1,2))
