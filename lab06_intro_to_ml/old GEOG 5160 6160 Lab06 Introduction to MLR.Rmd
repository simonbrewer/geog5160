...
This output gives the coefficients (the slope values) for each of the variables used. Most of these are straightforward to interpret (e.g. house price increases with the number of bedrooms), but others are less straightforward (e.g. house price decreases with the number of rooms). This arises from correlations between some of the variables, due to the fact that they are recording very similar information. The two most likely problems are:
...
We'll therefore remove `avg_rooms` and `households` and remake the model:
...
Now the coefficients are more logical: house prices increase with all other variables (house age, population, income and number of bedrooms). 
