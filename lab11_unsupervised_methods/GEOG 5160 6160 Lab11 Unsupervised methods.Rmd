...
```{r}
cluster.sp = sp::merge(borders, gap2.df, by.x = "ADM0_A3", by.y = "ISO3",
                      duplicateGeoms = TRUE)
```
...
```{r}
library(tmap)
tm_shape(cluster.sp) + tm_fill("kmeans", palette = "Set2")
```
...
Finally, we add the SOM cluster assignments back to the `cluster.sp` spatial object and map them out:

```{r}
gap2.df$somclust = as.factor(gap1.df$somclust)
cluster.sp = sp::merge(borders, gap2.df, by.x = "ADM0_A3", by.y = "ISO3",
                      duplicateGeoms = TRUE)
tm_shape(cluster.sp) + tm_fill("somclust", palette = "Set2")
```
