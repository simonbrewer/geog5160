...
To redisplay the original dem raster layer, add a new button called ('show-elev') which should be linked to the following procedure in your Netlogo code:
...
to setup-links
	...
	foreach gis:feature-list-of roads [
		foreach gis:vertex-lists-of ? [
			let n1 makenode first ?
			let n2 makenode last ?
...
- Loop through all the features in the roads layer
- Loop through all the vertices for a given feature from the road layer. The features are processed one at a time by the previous loop. The `?` represents the feature that is being processed by this loop.
- Create a node (`n1`) from the first vertex. The `?` now represents the current set of vertices
- Create a node (`n2`) from the second (last) vertex
- Make a link (a special type of turtle) between the two nodes
- End the procedure

The question mark is a very useful code shorthand. It is a temporary variable that represents the most recently created value. As we loop through the features, each feature is passed to the next command by the `?`. In the second (inner) loop we go through the sets of vertices, and now the `?` represents the set of vertices associated with the feature selected in the outer loop. 
