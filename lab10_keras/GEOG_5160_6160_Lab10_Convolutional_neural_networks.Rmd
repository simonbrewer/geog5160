...
The test_generatore stores the observed labels for the test set, so let's extract that and store it as `obs_class`:
...
This is an example of an image generator that will perform data augmentation. In each epoch, each image is transformed according to a set of random modifications. The parameters here set limits on the amount of transformation that any method will carry out
