...
To start with, we'll build a simple neural network with a single densely connected layer with 50 nodes between the input and output. We'll build a pretty simple model using the functional API with:

-   A input layer. Each observation going into the model is the set of 7 features (temperature, etc) from the current time step and the pollution from the previous step, which gives a shape of (8,)
...
Now compile and train. We'll add a callback that will save the model state after epoch, but only if the newest iteration is an improvement on the previous
...
## Define callback (save best iteration of model)
callbacks = list(
...
history <- model |> fit(
  ...
  callbacks = callbacks,
  shuffle = FALSE) # Note shuffle equal false as this is sequence data
```

Finally, we'll evaluate by predicting for the test set:
...
inputs <- keras_input(shape = dim(X_train)[2:3])
outputs <- inputs |>
  #layer_reshape(-1) |> 
...
Now train. I've set this to run silently (so it doesn't print output). If you want to track the training progress, set this to `verbose=1`

```{r}
## Define callback (save best iteration of model)
callbacks = list(
  callback_model_checkpoint("jena_lstm1.keras", 
                            monitor = "val_loss",
                            save_best_only = TRUE) 
)
...
history <- model |> fit(
  ...
  y = y_train, # sequence we're predicting
  batch_size = 64, # how many samples to pass to our model at a time
  epochs = 30, # how many times we'll look at the whole dataset
  validation_data = list(X_test, y_test),
  callbacks = callbacks,
  shuffle = FALSE,
  verbose = 0) # 
...
```{r}
model <- load_model("jena_lstm1.keras")
results <- model |> 
  evaluate(X_test, y_test, verbose = 0)
results
```
...
## Define callback (save best iteration of model)
callbacks = list(
  callback_model_checkpoint("jena_lstm2.keras", 
                            monitor = "val_loss",
                            save_best_only = TRUE) 
)
...
history <- model |> fit(
  ...
  y = y_train, # sequence we're predicting
  batch_size = 64, # how many samples to pass to our model at a time
  epochs = 50, # how many times we'll look at the whole dataset
  validation_data = list(X_test, y_test),
  callbacks = callbacks,
  shuffle = FALSE,
  verbose = 0) # 
...
```{r}
model <- load_model("jena_lstm2.keras")
results <- model |> 
  evaluate(X_test, y_test, verbose = 0)
results
```
...
outputs <- inputs |>
  #layer_reshape(-1) |> 
  layer_lstm(100, activation="relu",
...
## Define callback (save best iteration of model)
callbacks = list(
  callback_model_checkpoint("jena_lstm3.keras", 
                            monitor = "val_loss",
                            save_best_only = TRUE) 
)
...
```{r}
model <- load_model("jena_lstm3.keras")
results <- model |> 
  evaluate(X_test, y_test, verbose = 0)
results
```

In this case, adding the dropout makes the model slightly better. Bit it's likely that we're getting close to the limit of what can be improved on with this model (and given that we took a relatively small amount of data for training). We could try to train this model for longer (or slower) to see if that improves things, but we'll instead try one final experiment in which we make the model more complex.
...
## Define callback (save best iteration of model)
callbacks = list(
  callback_model_checkpoint("jena_lstm4.keras", 
                            monitor = "val_loss",
                            save_best_only = TRUE) 
)
...
```{r}
model <- load_model("jena_lstm4.keras")
results <- model |> 
  evaluate(X_test, y_test, verbose = 0)
results
```
...
The file *energydata_complete.csv* contains high-resolution temporal climate data (10 minute) from a station in Belgium, and has been used for energy forecasting. The file includes the following fields

- `date`: timestamp
- `Windspeed` wind speed (m/s)
- `Press_mm_hg`: pressure
- `RH_out`: relative humidity
- `T_out`: temperature (Celsius)

Your task for this exercise is to build an LSTM model to forecast the temperature variable one time step ahead. You can use the code below to convert the data into a format for recurrent models. Your answer should include:
- A description of the model architecture you have chosen
- Estimates of how well your model performs on the test set (e.g. RMSE, MAE)

GEOG 6160 Students: you should make at least one modification to your original model (e.g. dropouts, additional layers, etc). Describe the changes that you have made and how these have affected the performance metrics.
